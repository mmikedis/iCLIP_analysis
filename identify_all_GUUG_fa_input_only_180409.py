#### identify_all_GUUG_fa_input_only_180409.py
### 4/9/18
### Maria Mikedis mikedis@wi.mit.edu  

### this script outputs all 5' GUUG 3' sequences with a 35bp sequence
### the output is the position of the G of a 5' GUUG 3' sequence relative to the middle nucleotide in a 35 bp sequence (if the 35bp sequences are from CLIP data, the middle nucleotide is the crosslinked site)
...
while i<len(input):
	...
	while y==0:
		if len(sequence)>20 and sequence[y+17] == str("G") and sequence[y+18]==str("T") and sequence[y+19]==str("T") and sequence[y+20]==str("G") :	
		...
		y=y+1
	while y<18:	
		if len(sequence)>(20-y) and sequence[17-y] == str("G") and sequence[18-y]==str("T") and sequence[19-y]==str("T") and sequence[20-y]==str("G"):	
			new_line = [] ### delete content of previous new_line list
			new_line.append(read_line[0]) ### add sequence header to new line
			new_line.append(read_line[1]) ### add sequence to new line 
			start = int(17-y)
			...
			output.append(new_line)
			no_motif_found = False		
		if (y+20)<35 and len(sequence)>(20+y) and sequence[17+y] == str("G") and sequence[18+y]==str("T") and sequence[19+y]==str("T") and sequence[y+20]==str("G") :		
			new_line = [] ### delete content of previous new_line list
			new_line.append(read_line[0]) ### add sequence header to new line
			new_line.append(read_line[1]) ### add sequence to new line 
			start = int(17+y)
